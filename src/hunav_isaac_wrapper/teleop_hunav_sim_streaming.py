...
print("############################## Enabled WebRTC extension.")
import time
time.sleep(2)
# # Start Isaac Sim
# CONFIG = {
...
class TeleopHuNavSim(Node):
    ...
    def create_ros_camera_action_graph(
        self,
        camera_prim_path: str,
        image_topic: str,
        info_topic: str,
        graph_path: str = "/World/ROS2CameraLeft",
    ):
        print("### create_ros_camera_action_graph called ###", camera_prim_path)
        ...
        try:
            ...
            graph_params = {
                keys.CREATE_NODES: [
                    ...
                    # ("set_camera", "isaacsim.core.nodes.IsaacSetCamera"),

                    # 发布图像 / 相机信息
                    ("ros2_publish_image", "isaacsim.ros2.bridge.ROS2PublishImage"),
                    ("ros2_publish_camera_info", "isaacsim.ros2.bridge.ROS2PublishCameraInfo"),
                ],
                keys.CONNECT: [
                    ("on_playback_tick.outputs:tick", "ros2_publish_image.inputs:execIn"),
                    ("on_playback_tick.outputs:tick", "ros2_publish_camera_info.inputs:execIn"),

                    ("ros2_context.outputs:context", "ros2_publish_image.inputs:context"),
                    ("ros2_context.outputs:context", "ros2_publish_camera_info.inputs:context"),

                    # render product → publish image / camera info
                    ("create_render_product.outputs:renderProductPath", "ros2_publish_image.inputs:renderProductPath"),
                    ("create_render_product.outputs:renderProductPath", "ros2_publish_camera_info.inputs:renderProductPath"),
                ],
                keys.SET_VALUES: [
                    # context
                    ...
                    # camera selection
                    # ("set_camera.inputs:cameraPrim", camera_prim_path),
                    ("create_render_product.inputs:cameraPrim", camera_prim_path),
                    ("create_render_product.inputs:width", 640),
                    ("create_render_product.inputs:height", 480),

                    # topics
                    ("ros2_publish_image.inputs:topicName", image_topic),
                    ("ros2_publish_camera_info.inputs:topicName", info_topic),

                    # QoS：sensor data 典型 best effort
                    ("ros2_publish_image.inputs:qosProfile", "sensor_data"),
                    ("ros2_publish_camera_info.inputs:qosProfile", "sensor_data"),
                ],
            }

            og.Controller.edit({"graph_path": graph_path, "evaluator_name": "execution"}, graph_params)
            print(f"Successfully created ROS2 camera graph at {graph_path} for {camera_prim_path}")
        except Exception as e:
            print(f"Error creating ROS2 camera graph: {e}")
